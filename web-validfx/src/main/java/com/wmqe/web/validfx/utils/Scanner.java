...
public  class Scanner {
    ...
    private static Map<ValidationModel,Class<?>> validationModels = new HashMap<>();
    private static Map<String,Class<?>> validationModelMaps = new HashMap<>();
    private static List<Class<BaseValidator>> validatorTypes = new ArrayList<>();

    /**
     *
     * @return
     */
    public static List<Class<BaseValidator>> getValidatorTypes(){
        try {
            getValidationModels();
        } catch (Exception e) {
            return null;
    ...
    public static Class<?> getValidationModels(String key) {
        ...
        if(validationModelMaps.containsKey(key))
            return  validationModelMaps.get(key);
        return null;
    }

    public static  Map<ValidationModel,Class<?>> getValidationModels() throws Exception{
        if(validationModels.size() > 0)
            return  validationModels;
        synchronized (validationModels) {
            if (validationModels.size() == 0) {
                Set<Class<?>> clsList = getClasses("com.wmqe.web");
                if (clsList != null && clsList.size() > 0) {
                    for (Class<?> cls : clsList) {
                        ValidationModel validationModel = cls.getAnnotation(ValidationModel.class);
                        if (validationModel != null) {
                            validationModels.put(validationModel, cls);
                            validationModelMaps.put(validationModel.value(),cls);
                        }
                        if(BaseValidator.class.isAssignableFrom(cls)){
                            validatorTypes.add((Class<BaseValidator>)cls);
                        }
                    }
                }
            }
